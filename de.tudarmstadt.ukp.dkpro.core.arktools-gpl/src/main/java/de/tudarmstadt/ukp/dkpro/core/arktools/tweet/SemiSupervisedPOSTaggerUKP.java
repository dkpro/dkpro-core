...
import de.tudarmstadt.ukp.dkpro.core.arktools.tweet.POSFeatureTemplatesUKP.EmitFeatureTemplate;
import de.tudarmstadt.ukp.dkpro.core.arktools.tweet.POSFeatureTemplatesUKP.InterpolationFeatureTemplate;
import de.tudarmstadt.ukp.dkpro.core.arktools.tweet.POSFeatureTemplatesUKP.TransFeatureTemplate;
...
import edu.cmu.cs.lti.ark.ssl.pos.GradientGenSequenceModel;
import edu.cmu.cs.lti.ark.ssl.pos.GradientSequenceInterpolatedModel;
import edu.cmu.cs.lti.ark.ssl.pos.GradientSequenceModel;
import edu.cmu.cs.lti.ark.ssl.pos.POSModel;
import edu.cmu.cs.lti.ark.ssl.pos.POSUtil;
import edu.cmu.cs.lti.ark.ssl.pos.SemiSupervisedCRFHMMModel;
import edu.cmu.cs.lti.ark.ssl.pos.SupervisedGenSequenceModel;
import edu.cmu.cs.lti.ark.ssl.pos.SupervisedGenSequenceModelDiscObjective;
 ...
 */
public class SemiSupervisedPOSTaggerUKP {
	/**
	 * 
	 */
	private static final long serialVersionUID = 481162207516110632L;

	public static Logger log = Logger.getLogger(SemiSupervisedPOSTaggerUKP.class.getCanonicalName());
	...
	public static void main(String[] args) {
		...
		SemiSupervisedPOSTaggerUKP tagger = new SemiSupervisedPOSTaggerUKP(options);
		tagger.run();
	}

	public SemiSupervisedPOSTaggerUKP(POSOptionsUKP options0) {
		options = options0;
	...
	private void setVariousOptions() {
        ...
        embeddingsFile = (String) parser.getOptionValue(options.embeddingsFile);
        namesFile = (String) parser.getOptionValue(options.namesFile);
	...
	private Map<String, double[]> readDistSim() {
		...
		BufferedReader bReader = 
			BasicFileIO.openFileToRead(embeddingsFile);
		String line = BasicFileIO.getLine(bReader);
	...
	private void logObservationInfo() {
		...
		for (int[] observation : observations) {
			numTokens += observation.length;
			maxSeqLength = Math.max(maxSeqLength, observation.length);
		}

		if (useUnlabeledData) {
			for (int[] uObservation : uObservations) {
				numTokens += uObservation.length;
				maxSeqLength = Math.max(maxSeqLength, uObservation.length);
			}
		}
	...
	public List<Pair<Integer, Double>>[][] getActiveStackedFeatures(int numObservations, int numLabels, int startLabel, int stopLabel) {
		...
		POSFeatureTemplatesUKP templates1 = new POSFeatureTemplatesUKP();
...
//	modified this to fit the gaussian prior story
	...
	public void train() {
		...
		if (!useInterpolation) {
			transFeatures = 
				POSFeatureTemplatesUKP.getTransFeatures(useBiasFeature);
		} else { // interpolation of existing multinomials
			interpolationFeature =
				POSFeatureTemplatesUKP.getInterpolationFeatures(numHelperLanguages);
		}
		List<EmitFeatureTemplate> emitFeatures = 
			POSFeatureTemplatesUKP.getEmitFeatures(useStandardFeatures, 
