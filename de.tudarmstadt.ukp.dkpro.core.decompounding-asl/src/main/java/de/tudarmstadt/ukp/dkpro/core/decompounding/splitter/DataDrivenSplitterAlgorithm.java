	...
	implements SplitterAlgorithm
{
	 ...
	 */
	public DataDrivenSplitterAlgorithm(SimpleDictionary aDictionary,
	...
	/**
	 * Makes a single split on a given word. Returns all possible splittings.
	 * All splits consist of two elements
	 *
	 * @param aWord
	 * @return
	 */
	protected List<DecompoundedWord> makeSplit(String aWord)
	{
		...
		for (int i = aWord.length() - 3; i > -1; i--) {
			...
			backward[i] = backwardTrie.getSuccessors(new StringBuffer(
					subword).reverse().toString());
		}
		...
		for (int i = 1; i < diffForward.length - 1; i++) {
			if (diffForward[i - 1] < diffForward[i]
					&& diffForward[i] > diffForward[i + 1]) {
				maxForward[i - 1] = true;
			}
			else {
		...
		for (int i = diffBackward.length - 2; i > 0; i--) {
			if (diffBackward[i - 1] < diffBackward[i]
					&& diffBackward[i] > diffBackward[i + 1]) {
