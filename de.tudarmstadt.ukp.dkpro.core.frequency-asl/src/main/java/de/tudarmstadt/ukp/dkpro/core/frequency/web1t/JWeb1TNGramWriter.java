...
public class JWeb1TNGramWriter
    extends JCasAnnotator_ImplBase
{
        ...
        for (int level : cfd.getConditions()) {
            ...
            try {
                ...
                for (String key : cfd.getFrequencyDistribution(level).getKeys()) {
                    writer.write(key);
                    writer.write(TAB);
                    writer.write(new Long(cfd.getCount(level, key)).toString());
                    writer.write(LF);
                }
                writer.flush();
            }
    ...
    private HashMap<Integer, BufferedWriter> initializeWriters(int min, int max) throws ResourceInitializationException {
