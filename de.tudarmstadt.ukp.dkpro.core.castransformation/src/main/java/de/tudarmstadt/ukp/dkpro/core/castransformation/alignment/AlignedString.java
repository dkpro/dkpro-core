...
/**
 * Allows to stack strings on top of each other and modifying each of them
 * propagating changes up to the top while leaving lower levels unchanged.
 *
...
//	private static final  Log _log = LogFactory.getLog(AlignedString.class);
	...
	public
	AlignedString(
			final AlignedString underlying)
	{
		_underlying = underlying;
		_underlying.addChangeListener(this);

		_first._next = new ObliqueSegment(
				_first, _last,
				_underlying.getAnchor(0),
	...
	/**
	 * For the given interval on the current data, get the corresponding
	 * interval in the wrapped data.
	 *
	 * @param i
	 * @return
	 */
			...
			final Interval i)
	{
		...
		while (true) {
			if (cursor == null) {
				...
				if (
						(startSeg instanceof ObliqueSegment) &&
						(startSeg.length() >= i.getLength())
				) {
		...
		if (end < start) {
			throw new IllegalStateException(
					"BUG: End ["+end+"] of resolved interval before start ["+
					start+"]!");
		}
	 ...
	 * been added to UL
	 *
	 * Calling this method with start=22 end=30 should return [12, 18] as this
	 * is the interval 5 from UL plus the "Y" that has been inserted in AD.
	 *
	 * Generally:
	 * - if the start is within a deleted region, then find the next oblique
	 *   segment in AD to the right and return its start position.
	 * - if the end is within a deleted region, then find the next oblique
	 *   segment in AD to the left and return its end position.
	 *
	 * Anchors are always in UL. They are referenced from the ObliqueSegments
	 * in AD.
	 *
	 * @param i the interval on the underlying data.
	 * @return the corresponding interval in the view.
	 */
	public
	ImmutableInterval inverseResolve(
			final ImmutableInterval i)
	{
		if (_underlying == null) {
			return i;
		}
	...
	/**
	 * Get data segment currently at the given position. Anchors are never
	 * returned.
	 *
	 * @param position
	 ...
	 */
	public
	AbstractDataSegment getSegmentAt(
			final int position)
	{
	...
	/**
	 * Fetch the segment that includes the designated offset position. The
	 * segment is searched from left to right. If the position is at the the
	 * start of a segment and the parameter {@code includeAnchors} is {@code
	 * true}, then the anchor is returned instead of the segment or if the
	 * position is right one beyond the end of the data, then the end
	 * boundary anchor is returned.
	 *
	 * @param position the offset.
	 * @param includeAnchors whether or not to include anchors.
	 * @return
	 */
	private
	AbstractDataSegment getSegmentAt(
			final int position,
			final boolean includeAnchors)
	{
		if (position < 0) {
		...
		if (seg == null) {
			if ((includeAnchors) && (pEnd == position)) {
				...
				throw new IndexOutOfBoundsException(
						"Index ["+position+"] not in range [0-"+pEnd+
						"], ["+(position - pEnd)+"] off");
			}
		}
		...
		if (includeAnchors) {
			if (
					seg._prev.isAnchor() &&
					(((Anchor) seg._prev).getPosition() == position)
			) {
				return seg._prev;
			}
		}
...
//		dropSuperflourous(suffix);
	...
	/**
	 * If the given segment is a zero-length base segment, then it can be
	 * dropped. Zero-length oblique segments need to be retained because
	 * through them we can know e.g. if we extended a word. In the following
	 * example the empty oblique segments allows us to do an inverse resolve
	 ...
	 */
	private
	void dropSuperflourous(
			final AbstractDataSegment seg)
	{
		if (
				(seg instanceof BaseSegment) &&
//				!seg.isAnchor() &&
				(seg.length() == 0)
		) {
...
//		}
	...
	/**
	 * Get an anchor at the specified position. Breaks up the segment at the
	 * given point if necessary. If there already is an anchor, it is reused.
	 *
	 * @param pos
	 * @return
	 */
			...
			final int pos)
	{
		...
		if (prefix.isAnchor()) {
			...
			// Insert segment
			final AnchorSegment seg = new AnchorSegment(
					prefix, suffix);
			prefix._next = seg;
			suffix._prev = seg;
	 ...
	 */
	public
	Collection<DataSegment> getSegments()
	{
	...
	/**
	 * Create a string representation of the segents. This is for debugging
	 * purposes.
	 *
	 * @return
	 */
	public
	String dataSegmentsToString()
	{
		final StringBuilder sb = new StringBuilder();
		sb.append(">>");
		for (final DataSegment s : this) {
...
//		sb.append("} ");
	...
	implements AlignedString.DataSegment
	{
		...
		public
		AbstractDataSegment(
				final AbstractDataSegment prev,
				final AbstractDataSegment next)
		{
			_prev = prev;
			_next = next;
		}
	...
	extends AbstractDataSegment
	{
				...
				final int position)
		{
			...
			// Create new segment
			final BaseSegment suffix = new BaseSegment(
					this, _next, _data.substring(pos, _data.length()));
	...
	extends AbstractDataSegment
	{
				...
				final int position)
		{
			...
			final int pos = position - getStart();
			final Anchor splitAnchor = _underlying.getAnchor(
					_start.getPosition() + pos);

			// Create new segment
			final ObliqueSegment suffix = new ObliqueSegment(
					this, _next, splitAnchor, _end);
...
implements Iterator<AlignedString.DataSegment> {
	...
	public
	DataSegmentIterator(
			final AlignedString.AbstractDataSegment first)
	{
		_next = first;
		_includeAll = false;
	}
	...
	@Override
	public
	AlignedString.DataSegment next()
	{
