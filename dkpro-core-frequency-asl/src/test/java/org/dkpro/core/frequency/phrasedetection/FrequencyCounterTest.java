...
{
    @Test
    public void testCount(@TempDir File tempDir) throws Exception
    {
        int minCount = 1;

        File targetFile = new File(tempDir, "counts.txt");
        ...
        AnalysisEngineDescription writer = createEngineDescription(FrequencyWriter.class,
                FrequencyWriter.PARAM_TARGET_LOCATION, targetFile, FrequencyWriter.PARAM_MIN_COUNT,
                minCount);
        ...
        assertArrayEquals(Files.lines(expectedFile.toPath()).sorted().toArray(),
                Files.lines(targetFile.toPath()).sorted().toArray());
    }

    @Test
    public void testCountSortedAlphabetically(@TempDir File tempDir) throws Exception
    {
        int minCount = 1;
        ...
        assertArrayEquals(Files.readAllBytes(expectedFile.toPath()),
                Files.readAllBytes(targetFile.toPath()), "Alphabetic sorting invalid.");
    }

    @Test
    public void testCountSortedByValue(@TempDir File tempDir) throws Exception
    {
        int minCount = 1;
        ...
        double[] unigrams = Files.lines(targetFile.toPath())
                ...
                .map(fields -> fields[1]).mapToDouble(Double::parseDouble).toArray();
    ...
    @Test
    public void testSortBoth() throws IOException, UIMAException
    {
        String sentence = "This is a first test that contains a first test example";
        String language = "en";
