...
public class DKPro2Lxf
{
     ...
     *            the target LXF.
     * @param toolName
     *            the name of the tool that inserts annotation
     */
    public static void convert(JCas aJCas, LxfGraph aSource, LxfGraph aTarget,
            Map<String, String> ids)
    {
        ...
        ToolIndex toolEdgeIndex = new ToolIndex(ids.values());
        ToolIndex toolNodeIndex = new ToolIndex(ids.values());
        ToolIndex toolRegionIndex = new ToolIndex(ids.values());
        ...
        for (Sentence sentence : select(aJCas, Sentence.class)) {
            ...
            System.out.println("Sentence: " + toolid);
            ...
            if (aSource == null || needsExport(aJCas, sentence)) {
                ...
                // Sentence region
                LxfRegion sentenceRegion = new LxfRegion(toolid, toolRegionIndex.index(toolid),
                        sentence.getBegin(), sentence.getEnd());
                aTarget.addRegion(sentenceRegion);
                sentenceNode = new LxfNode(LAYER_SENTENCE, toolid, toolNodeIndex.index(toolid), 0,
                        sentenceRegion);
            ...
            for (Token token : tokens) {
                ...
                if (aSource == null || needsExport(aJCas, token)) {
                    LxfRegion tokenRegion = new LxfRegion(toolid, toolRegionIndex.index(toolid),
                    ...
                    int edgeIndex = toolEdgeIndex.index(toolid);
                ...
                if (pos != null) {
                    if ((aSource == null || needsExport(aJCas, pos))) {
                        ...
                        aTarget.addEdge(new LxfEdge(morphNode.getOrigin(),
                                toolEdgeIndex.index(toolid), 0, morphNode, tokenNode));
                ...
                if (lemma != null && (aSource == null || needsExport(aJCas, lemma))) {
                    ...
                    if (lemmaNode == null) {
                        ...
                        aTarget.addEdge(new LxfEdge(lemmaNode.getOrigin(),
                                toolEdgeIndex.index(toolid), 0, lemmaNode, tokenNode));
            ...
            for (Dependency dep : deps) {
                ...
                LxfNode depNode = new LxfNode(LAYER_DEPENDENCY, toolid, toolNodeIndex.index(toolid),
                        0);
                ...
                aTarget.addEdge(new LxfEdge(depNode.getOrigin(), toolEdgeIndex.index(toolid), 0,
                        depNode, depMorphNode));

                if (!govMorphNode.getId().equals(depMorphNode.getId())) {
                    aTarget.addEdge(new LxfEdge(depNode.getOrigin(), toolEdgeIndex.index(toolid), 0,
                            govMorphNode, depNode));
