...
/**
 * Removes all tokens/lemmas/stems (depending on the "Mode" setting) that do not match the given parts of speech.
 *
 * @author zesch
 *
 */
public class PosFilter extends JCasAnnotator_ImplBase {
     ...
     */
    public static final String PARAM__TYPE_TO_REMOVE  = "TypeToRemove";
    @ConfigurationParameter(name=PARAM__TYPE_TO_REMOVE, mandatory=true)
	...
	public void process(JCas jcas) throws AnalysisEngineProcessException {
        ...
        if (posType == null) {
            throw new AnalysisEngineProcessException(new Throwable("Could not get type for feature path: " + typeToRemove));
        }

		List<AnnotationFS> toRemove = new ArrayList<AnnotationFS>();
        try {
            for (Entry<AnnotationFS, String> entry: FeaturePathFactory.select(jcas.getCas(), typeToRemove)) {
    ...
    private AnnotationFS getPos(Type type, AnnotationFS fs) {
        ...
        if (posAnnotations.size() != 1) {
            getContext().getLogger().log(Level.WARNING, "Could not find matching POS annotation for annotation: " + fs.getCoveredText());
            return null;
        }
