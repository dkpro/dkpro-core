...
/**
 * Utility class for doing deep copies of FeatureStructures from one CAS to
 * another. To handle cases where the source CAS has multiple references to the
 * same FS, you can create one instance of CasCopier and use it to copy multiple
...
public class CasCopier
{
	...
	/**
	 * Creates a new CasCopier that can be used to copy FeatureStructures from
	 * one CAS to another. Note that if you are merging data from multiple
	 * CASes, you must create a new CasCopier for each source CAS.
	 *
	 * @param aSrcCas the CAS to copy from.
	 * @param aDestCas the CAS to copy into.
	 */
	public CasCopier(CAS aSrcCas, CAS aDestCas)
	{
		...
		mDestSofaFeature = aDestCas.getTypeSystem().getFeatureByFullName(
				CAS.FEATURE_FULL_NAME_SOFA);
	}

	/**
	 * Does a complete deep copy of one CAS into another CAS. The contents of
	 * each view in the source CAS will be copied to the same-named view in the
	 * destination CAS. If the view does not already exist it will be created.
	 * All FeatureStructures that are indexed in a view in the source CAS will
	 * become indexed in the same-named view in the destination CAS.
	 *
	 * @param aSrcCas the CAS to copy from
	 * @param aDestCas the CAS to copy to
	 * @param aCopySofa if true, the sofa data and mimeType of each view will be
	 *            copied. If false they will not.
	 */
	...
	/**
	 * Does a deep copy of the contents of one CAS View into another CAS. If a
	 * view with the same name as <code>aSrcCasView</code> exists in the
	 * destination CAS, then it will be the target of the copy. Otherwise, a new
	 * view will be created with that name and will become the target of the
	 * copy. All FeatureStructures that are indexed in the source CAS view will
	 * become indexed in the target view.
	 *
	 * @param aSrcCasView the CAS to copy from
	 * @param aCopySofa if true, the sofa data and mimeType will be copied. If
	 * 			false they will not.
	 */
	public void copyCasView(CAS aSrcCasView, boolean aCopySofa)
	{
		...
		if (aCopySofa) {
			...
			if (aSrcCasView.getDocumentText() != null) {
				targetView.setSofaDataString(aSrcCasView.getDocumentText(),
						sofaMime);
			}
			else if (aSrcCasView.getSofaDataURI() != null) {
				targetView.setSofaDataURI(aSrcCasView.getSofaDataURI(),
						sofaMime);
			}
			else if (aSrcCasView.getSofaDataArray() != null) {
				targetView.setSofaDataArray(copyFs(aSrcCasView
						.getSofaDataArray()), sofaMime);
			}
		}
			...
			while (iter.hasNext()) {
				...
				if (!indexedFs.contains(fs)) {
					...
					if (fs instanceof AnnotationFS) {
						...
						if (sofa == null) {
							copyOfFs.setFeatureValue(mDestSofaFeature,
									targetView.getSofa());
						}
					}
	 ...
	 *
	 * @param aFS the FS to copy. Must be contained within the source CAS.
	...
	public FeatureStructure copyFs(FeatureStructure aFS)
	{
		...
		assert ((CASImpl) aFS.getCAS()).getBaseCAS() == ((CASImpl) mSrcCas)
				.getBaseCAS();
	 ...
	 * </P>
	 *
	 * <strong>_IF_ there is no appropriate
	 * alternative present in the destination CAS. If the destination CAS
	 * contains an annotation that is identical to the referenced annotation in
	 * the sourceCas except it has a different ID, then it won't be deep copied,
	 * but the reference-id in the annotation will be updated.</strong>
	 *
	 * @param aSrcFS FeatureStructure to copy from
	 * @param aDestFS FeatureStructure to copy to
	 */
	private void copyFeatures(FeatureStructure aSrcFS, FeatureStructure aDestFS)
	{
		...
		for (Feature srcFeat : srcType.getFeatures()) {
			...
			if (srcFeat.getRange().isPrimitive()) {
				...
				aDestFS.setFeatureValueFromString(destFeat, aSrcFS
						.getFeatureValueAsString(srcFeat));
			}
			else {
				// recursive copy
	...
	/**
	 * Performs batch-copying of Annotations (could also be generalized to
	 * FeatureStructures)
	 *
	 * While copying the annotations, the whole batch is held in a class
	 * attribute. This way, we can cope with referenced annotations that have
	 * not been copied/recovered yet.
	 *
	 * @param annoList the list of annotations that is to be batch-copied
