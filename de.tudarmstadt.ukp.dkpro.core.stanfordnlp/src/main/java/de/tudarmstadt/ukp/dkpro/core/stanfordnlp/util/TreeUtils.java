...
public class TreeUtils
{
	...
	/**
	 * Recursively creates an edu.stanford.nlp.trees.Tree from a ROOT annotation
	 * It also saves the whitespaces before and after a token as
	 * <code>CoreAnnotation.BeforeAnnotation</code> and
	 * <code>CoreAnnotation.AfterAnnotation</code> in the respective label of
	 * the current node.
	 *
	 * @author Oliver Ferschke
	 *
	 * @param root
	 *            the ROOT annotation
	 * @return an edu.stanford.nlp.trees.Tree object representing the syntax
		...
		throws CASException
	{
		...
		// before we can create a node, we must check if we have any children
		// (we have to know whether to create a node or a leaf - not very
		// dynamic)
		if (root instanceof Constituent && !isLeaf((Constituent) root)) {
			...
			// create information about preceding and trailing whitespaces in
			// the leaf node
			StringBuffer preWhitespaces = new StringBuffer("");
			StringBuffer trailWhitespaces = new StringBuffer("");

			List<Token> precedingTokenList = selectPreceding(
					aJCas, Token.class, wordAnnotation, 1);
			List<Token> followingTokenList = selectFollowing(
					aJCas, Token.class, wordAnnotation, 1);

			if (precedingTokenList.size() > 0) {
				Token precedingToken = precedingTokenList.get(0);
				int precedingWhitespaces = wordAnnotation.getBegin()
						- precedingToken.getEnd();
			...
			if (followingTokenList.size() > 0) {
				...
				int trailingWhitespaces = followingToken.getBegin()
			...
			// write whitespace information as CoreAnnotation.BeforeAnnotation
			// and CoreAnnotation.AfterAnnotation to the node
			// add annotation to list and write back to node label
			((CoreLabel) wordNode.label()).set(
					CoreAnnotations.BeforeAnnotation.class, preWhitespaces
							.toString());
			((CoreLabel) wordNode.label()).set(
					CoreAnnotations.AfterAnnotation.class, trailWhitespaces
							.toString());

			// get POS-annotation
			// get the token that is covered by the POS
			List<POS> coveredPos = JCasUtil.selectCovered(aJCas,
					POS.class, wordAnnotation);
			...
			rootNode = tFact.newTreeNode(pos.getPosValue(), Arrays
					.asList((new Tree[] { wordNode })));
		}
	...
	/**
	 * <p>
	 * Recreates a Stanford Tree from the StanfordParser annotations and saves
	 * all non-StanfordParser-Annotations within the scope of the sentence in
	 * the label of the best fitting node.
	 * </p>
	 *
	 * <p>
	 * <strong>CAUTION: </strong><i>This method is intended for the use by CAS
	 * Multipliers, which create new CASes from this tree. The annotation-spans
	 * in the source-CAS will be changed!!!!!! You do NOT want to use the source
	 * CAS after this method has been called. The createStanfordTree()-method
	 * does not change the CAS, so use this instead, if the annotations do not
	 * have to be recovered or accessed in the tree.</i>
	 * </p>
	 *
	 * <p>
	 * TODO: This behavior could be changed by making COPIES of the annotations
	 * and changing the copied instead of the originals. However, in order to
	 * being able to make copies, a dummy CAS must be introduced to which the
	 * annotations can be copied. When they are recovered, they will be copied
	 * to the new destination CAS anyway.
	 * </p>
	 *
	 * @param root
	 *            the ROOT annotation
		...
		throws CASException
	{
		...
		// Using getCoveredAnnotations instead of iterate, because subiterators
		// did not work in all cases
		List<Annotation> annosWithinRoot = JCasUtil.selectCovered(
				aJCas, Annotation.class, root);
		...
		/*
		 * Add annotations to the best-fitting nodes. The best-fitting node for
		 * an annotation is the deepest node in the tree that still completely
		...
		for (Annotation curAnno : nonParserAnnotations) {
			...
			if (bestFittingNode != null) {
				...
				// save annotation text before translating spans
				String coveredText = curAnno.getCoveredText();
				...
				// which
				// we store UIMA annotations or create it, if it does not exist
				Collection<Annotation> annotations = ((CoreLabel) bestFittingNode
						.label()).get(UIMAAnnotations.class);
				if (annotations == null) {
					annotations = new ArrayList<Annotation>();
				}

				// add annotation + checksum of annotated text
				// to list and write it back to node label
				annotations.add(curAnno);

				((CoreLabel) bestFittingNode.label()).set(
						UIMAAnnotations.class, annotations);
			}
		}
	...
	{
		return (constituent.getChildren() == null || constituent.getChildren()
				.size() == 0);
	}
	...
	public static String tree2Words(Tree t)
	{
		...
		for (Tree leaf : leaves) {
			String word = ((CoreLabel) leaf.label())
					.get(CoreAnnotations.ValueAnnotation.class);

			// TODO maybe double check preceding whitespaces, because
			// transformations
			// could have resulted in the situation that the trailing
			// whitespaces of out last
			// tokens is not the same as the preceding whitespaces of out
			// current token
			// BUT: This has also to be done in getTokenListFromTree(...)

			// now add the trailing whitespaces
			String trailingWhitespaces = ((CoreLabel) leaf.label())
					.get(CoreAnnotations.AfterAnnotation.class);
			// if no whitespace-info is available, insert a whitespace
			// this may happen for nodes inserted by TSurgeon operations
	 ...
	 * This method reindexes already indexed trees starting with index 1. The
	 * method expects trees with <code>CoreMap</code>-type labels.
	 *
	 ...
	 *
	 * @param pennString
	 *            A Penn Treebank-style String as produced by the
	 *            StandfordParser
