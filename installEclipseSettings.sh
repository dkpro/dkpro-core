...
JDT_CORE_PREFS="dkpro-core-build/src/main/resources/dkpro-core/eclipse/org.eclipse.jdt.core.prefs"
JDT_UI_PREFS="dkpro-core-build/src/main/resources/dkpro-core/eclipse/org.eclipse.jdt.ui.prefs"

if [ ! -f "$JDT_CORE_PREFS" ] || [ ! -f "$JDT_UI_PREFS" ]; then
  echo "Run from the dkpro-core repository root (next to dkpro-core-build/)." >&2
  exit 1
fi
...
for module in */; do
  module="${module%/}"
  [ "$module" = "dkpro-core-build" ] && continue
