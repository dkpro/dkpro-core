...
implements Interval
{
	public static
	Interval grow(
			final Collection<? extends Interval> ivals)
	{
		if (ivals.size() == 0) {
			return null;
		}
	...
	/**
	 * Returns true if the current includes the given interval. The inclusion
	 * does not have to be proper, so equal boundaries are allowed.
	 *
	 * @param i
	 * @return
	 */
